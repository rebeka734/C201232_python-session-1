...
def check_temperature(readings, threshold):
    for temp in readings:
        if temp > threshold:
            print(f"Warning: Temperature {temp}°C exceeds the threshold!")
# Example list of temperature readings
temperature_readings = [22, 29, 35, 18, 40, 25, 31]
threshold = 30  # Threshold temperature
# Call the function with the list of temperatures
check_temperature(temperature_readings, threshold)
...
def check_stock(products):
    for product, details in products.items():
        ...
        if stock < min_required:
            print(
                f"Restock alert: {product.capitalize()} is low on stock. Only {stock} left. Minimum required is {min_required}.")
        else:
            print(f"{product.capitalize()} has sufficient stock. {stock} in stock.")
